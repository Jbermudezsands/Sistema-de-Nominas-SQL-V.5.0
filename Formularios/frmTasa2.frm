...
Begin VB.Form frmTasa2 
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin TrueOleDBGrid80.TDBGrid DBGrTasas 
         ...
         _StyleDefs(21)  =   "Splits(0).CaptionStyle:id=22,.parent=4,.bgcolor=&H808000&,.fgcolor=&H0&,.bold=-1"
         ...
         _StyleDefs(24)  =   "Splits(0).HeadingStyle:id=14,.parent=2,.alignment=2,.bgcolor=&HF58154&"
...
Me.datPrimaryRS.RecordSource = "SELECT  * From CambioMoneda ORDER BY FechaDia"
...
sql = "SELECT  FechaDia, MontoDia From CambioMoneda " & _
"WHERE     (FechaDia = '" & Format(Fecha, "yyyymmdd") & "') " & _
"ORDER BY FechaDia "
...
If Not datPrimaryRS.Recordset.EOF Then
   Fecha = Format(datPrimaryRS.Recordset("FechaDia"), "dd/mm/yyyy")
   
    Encontrado = True
    Cambio = datPrimaryRS.Recordset("MontoDia")
   ' MDIPrimero.StatusBar2.Panels(2) = "Tasa Cordobas: " & Format(Cambio, "##,##0.00") & "    " & "Tasa Libras: " & Format(datPrimaryRS.Recordset("MontoLibras"), "##,##0.00")
    MDIPrimero.StatusBar2.Panels(2) = "Tasa Cordobas: " & Format(Cambio, "##,##0.00")
End If
