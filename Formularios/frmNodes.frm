...
If llave = -1 Then
    sql = "INSERT INTO [dbo].[Actividades]([Raiz], [Codigo], [Actividad], Pagacliente) " & _
            "Values (" & raiz & ", " & Trim(Me.txtId) & ", '" & Trim(Me.txtDesc) & "'," & IIf(Me.chkPago(1).Value, 1, 0) & ")"
    cnx.Execute sql
    
    sql = "SELECT max([Llave]) AS llave " & _
    ...
    With frmActividades.tvActividades
        If raiz = 0 Then
            .Nodes.Add , tvwLast, "A" & Trim(Str(rs!llave)), Trim(Me.txtId) & ".- " & Trim(Me.txtDesc), 1, 2
        Else
            .Nodes.Add "A" & Trim(Str(raiz)), tvwChild, "A" & Trim(Str(rs!llave)), Trim(Me.txtId) & ".- " & Trim(Me.txtDesc), 1, 2
        End If
        .Nodes.item(.Nodes.Count).Selected = True
    End With
Else
    sql = "UPDATE Actividades SET Codigo = " & Trim(Me.txtId) & ", [Actividad] = '" & Trim(Me.txtDesc) & "', Pagacliente = " & IIf(Me.chkPago(1).Value, 1, 0) & _
            " WHERE codigo = " & codigo & " AND raiz = " & raiz
    cnx.Execute sql
    
    With frmActividades.tvActividades.SelectedItem
        .Text = Trim(Me.txtId) & ".- " & Trim(Me.txtDesc)
    End With
End If
...
Public Property Let prCodigo(ByVal val As Integer)
codigo = val
End Property
