...
Begin VB.Form frmExtrasAutorizar 
   ...
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel1 
      Height          =   255
      Left            =   240
      OleObjectBlob   =   "frmExtrasAutorizar.frx":0000
      TabIndex        =   1
      Top             =   240
      Width           =   1215
   End
   Begin MSComctlLib.ImageList ilnode 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmExtrasAutorizar.frx":006C
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmExtrasAutorizar.frx":0830
            Key             =   ""
         EndProperty
      EndProperty
   End
   ...
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel2 
      Height          =   255
      Left            =   240
      OleObjectBlob   =   "frmExtrasAutorizar.frx":1640
      TabIndex        =   7
      Top             =   3360
      Width           =   1575
   End
   ...
   End
End
Attribute VB_Name = "frmExtrasAutorizar"
        ...
        If rs2!horas = 0 Then
            sql = "INSERT INTO HORASEXTRAS " & _
                    ...
                    " canthoras = (SELECT  SUM(ROUND(DATEDIFF(MI, CAST(SUBSTRING(CAST(AP.HSALIDA AS NVARCHAR(19)),1,11) + SUBSTRING(CAST(N.HSALIDA AS NVARCHAR(19)),12,8) AS DATETIME),  AP.HSALIDA) / 60.0,2)) " & _
                                    "FROM   EMPLEADO E INNER JOIN ACTIVIDADESPRODUCCION AP ON E.CODEMPLEADO = AP.EMPLEADO " & _
...
sql = "SELECT   CodEmpleado1 AS Número, 'Empleado' =    case when Nombre2 is null then Nombre1 + ' ' + Apellido1 + ' ' + Apellido2 when Nombre2 = '' then Nombre1 + ' ' + Apellido1 + ' ' + Apellido2 else Nombre1 + ' ' + Nombre2 + ' ' + Apellido1 + ' ' + Apellido2 end, " & _
      "              AP.Fecha, CAST(SUBSTRING(CAST(AP.HSALIDA AS NVARCHAR(19)),1,11) + SUBSTRING(CAST(N.HSALIDA AS NVARCHAR(19)),12,8) AS DATETIME) AS Parámetro, " & _
      "              AP.HSALIDA as [H. Salida], " & _
      "              HE = ROUND(DATEDIFF(MI, CAST(SUBSTRING(CAST(AP.HSALIDA AS NVARCHAR(19)),1,11) + SUBSTRING(CAST(N.HSALIDA AS NVARCHAR(19)),12,8) AS DATETIME), " & _
      "              AP.HSALIDA) / 60.0,2), AP.HEXTRAS AS Aprobar, " & _
      "              E.CodEmpleado, AP.FECHAAPROBADO AS Aprobado " & _
        "FROM    EMPLEADO E " & _
        "            INNER JOIN ACTIVIDADESPRODUCCION AP ON E.CODEMPLEADO = AP.EMPLEADO " & _
        "            INNER JOIN TIPONOMINA N ON E.CODTIPONOMINA = N.CODTIPONOMINA " & _
        "WHERE  N.CodTipoNomina = '" & Mid(Me.tvNominas.SelectedItem.Key, 2) & "' AND AP.Eliminar = 1 AND " & _
        "       AP.FECHA BETWEEN ISNULL((select FECHANOMINAINI from nomina where codtiponomina = '" & Mid(Me.tvNominas.SelectedItem.Key, 2) & "' AND FECHANOMINA = (SELECT DATEADD(d, -1, FECHANOMINAINI) FROM NOMINA WHERE CODTIPONOMINA = '" & Mid(Me.tvNominas.SelectedItem.Key, 2) & "' AND ACTIVA = 1)), " & _
