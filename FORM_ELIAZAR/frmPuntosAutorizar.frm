...
Do While Not rs3.EOF
    If rs3!aprobar <> rs4!aprobar Then
        sql = "UPDATE EMPLEADO SET SUELDOPERIODO = CASE WHEN Dolarizado = 1 THEN SueldoPeriodo ELSE " & _
                    ...
                    "            CANTPTS = (SELECT ISNULL(SUM(CANTPTS),0) FROM PUNTOSEMPLEADO PE INNER JOIN PUNTOS P ON PE.PUNTOS = P.ID WHERE PE.APROBADO = 1 AND EMPLEADO = " & rs3!CodEmpleado & ")) DAT) END, " & _
