...
Begin VB.Form frmPuntosSolicitudes 
   ...
   Begin VB.Frame fraSolicitud 
      ...
      Begin ACTIVESKINLibCtl.SkinLabel SkinLabel10 
         Height          =   375
         Left            =   120
         OleObjectBlob   =   "frmPuntosSolicitudes.frx":0440
         TabIndex        =   19
         Top             =   720
         Visible         =   0   'False
         Width           =   975
      ...
      Begin ACTIVESKINLibCtl.SkinLabel SkinLabel13 
         Height          =   255
         Left            =   120
         OleObjectBlob   =   "frmPuntosSolicitudes.frx":04B8
         TabIndex        =   20
         Top             =   1440
         Visible         =   0   'False
         Width           =   975
      ...
      Begin ACTIVESKINLibCtl.SkinLabel SkinLabel14 
         Height          =   375
         Left            =   120
         OleObjectBlob   =   "frmPuntosSolicitudes.frx":0528
         TabIndex        =   22
         Top             =   240
         Visible         =   0   'False
         Width           =   1335
   ...
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel1 
      Height          =   255
      Left            =   240
      OleObjectBlob   =   "frmPuntosSolicitudes.frx":05AA
      TabIndex        =   14
      Top             =   240
      Width           =   1215
   End
   Begin MSComctlLib.ImageList ilnode 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmPuntosSolicitudes.frx":0616
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmPuntosSolicitudes.frx":0DDA
            Key             =   ""
         EndProperty
      EndProperty
   End
   ...
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel2 
      Height          =   255
      Left            =   240
      OleObjectBlob   =   "frmPuntosSolicitudes.frx":1BEA
      TabIndex        =   15
      Top             =   3360
      Width           =   1215
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel12 
      Height          =   255
      Left            =   5040
      OleObjectBlob   =   "frmPuntosSolicitudes.frx":1C54
      TabIndex        =   37
      Top             =   3360
      Width           =   3375
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel15 
      Height          =   255
      Left            =   5040
      OleObjectBlob   =   "frmPuntosSolicitudes.frx":1CEE
      TabIndex        =   38
      Top             =   6480
      Width           =   3375
   End
...
Else
    ...
    ElseIf Trim(Me.txtJustificacion) = "" Then
        ...
        Exit Sub
    ElseIf Trim(Me.txtDocumento) = "" Then
        MsgBox "Digite el documento que avala los puntos", vbInformation
        Me.txtDocumento.SetFocus
        Exit Sub
    End If
    
    'CODIGO PARA COPIAR LOS ARCHIVOS
    If Trim(Me.txtdirdoc) <> "" Then
        Dim archivo As New FileSystemObject
        If archivo.FileExists(App.Path & "\DOCUMENTOS\" & Me.txtdirdoc.Tag) Then _
            If MsgBox("El archivo ya existe, desea reemplazarlo", vbYesNo, "SACC") = vbNo Then Exit Sub
'        If Not archivo.FolderExists(App.Path & "\DOCUMENTOS") Then _
'            archivo.CreateFolder (App.Path & "\DOCUMENTOS")
        If Not archivo.FolderExists(App.Path & "\DOCUMENTOS\" & rs3!CodEmpleado) Then _
            archivo.CreateFolder (App.Path & "\DOCUMENTOS\" & rs3!CodEmpleado)
        
'        archivo.CopyFile Me.txtdirdoc.Text, App.Path & "\DOCUMENTOS\" & Me.txtdirdoc.Tag
        archivo.CopyFile Me.txtdirdoc.Text, App.Path & "\DOCUMENTOS\" & rs3!CodEmpleado & "\" & Me.txtdirdoc.Tag
        MsgBox "Se ha copiado el DOCUMENTO en: " & vbCr & App.Path & "\DOCUMENTOS\", vbInformation
    End If
    ...
    sql = "INSERT INTO PUNTOSEMPLEADO(EMPLEADO, PUNTOS, APROBADO, FECHASOLICITUD, JUSTIFICACION, DOCUMENTO, DIRDOCUMENTO, ELIMINAR) VALUES(" & _
                ...
                ", 0, '" & Format$(Me.dtpSolicitud.Value, "yyyymmdd") & "', '" & Me.txtJustificacion.Text & "', '" & Me.txtDocumento.Text & "', '" & Mid(Me.txtdirdoc.Tag, 1, Len(Me.txtdirdoc.Tag) - 1) & "', 1)"
    cnx.Execute sql
...
Else
    ...
    With OFName
        ...
        .lpstrFilter = "Archivos PDF (*.pdf)" + Chr$(0) + "*.pdf" + Chr$(0) + _
                        "Archivos IMAGENES (*.jpg, *.jpeg, *.jpe, *.bmp)" + Chr$(0) + "*.jpg;*.jpeg;*jpe;*.bmp" + Chr$(0) + _
                        "Archivos WORD (*.doc, *.docx)" + Chr$(0) + "*.doc;*.docx" + Chr$(0) + _
    ...
    If GetOpenFileName(OFName) Then
        Me.txtdirdoc.Text = Trim$(OFName.lpstrFile)
        Me.txtdirdoc.Tag = Trim$(OFName.lpstrFileTitle)
    Else
        Me.txtdirdoc = ""
    End If
End If
...
If MsgBox("Esta seguro de guardar los cambios", vbYesNo) = vbYes Then
    ...
    sql = "UPDATE [dbo].[Empleado] " & _
            "SET [SalPorcentaje] = " & val(Me.txtPorcentaje) & ", [SueldoPeriodo] = CASE WHEN Dolarizado = 1 THEN SueldoPeriodo ELSE " & val(Me.txtTotal) & " END " & _
...
Private Sub tdbgEmpleados_FilterChange()
...
On Error Resume Next
Set cols = tdbgEmpleados.Columns
Dim c As Integer

c = tdbgEmpleados.col
tdbgEmpleados.HoldFields
sql = rs3.Filter
rs3.Filter = getFilter(col, cols, rs3)

tdbgEmpleados.col = c
tdbgEmpleados.EditActive = True
End Sub
...
sql = "SELECT  P.ID, G.GRUPO, P.DESCRIPCION, P.CANTPTS AS PUNTOS, EP.APROBADO, EP.FECHASOLICITUD, EP.FECHAAPROBADO, E.CODEMPLEADO, EP.JUSTIFICACION, " & _
...
Dim X As Integer
For X = 7 To Me.tdbgPuntos.Columns.Count - 1
...
sql = rs4.Filter
rs4.Filter = getFilter(col, cols, rs4)

tdbgPuntos.col = c
tdbgPuntos.EditActive = True
End Sub

Private Function getFilter(col As TrueOleDBGrid80.Column, cols As TrueOleDBGrid80.Columns, Optional rsc As ADODB.Recordset) As String
...
For Each col In cols
    If Trim(col.FilterText) <> "" Then
        ...
        Select Case rsc.Fields(X).Type
        Case adVarWChar, adVarChar: tmp = tmp & "[" & col.DataField & "] LIKE '%" & col.FilterText & "%'"
        Case adInteger, adNumeric, adDouble: tmp = tmp & "[" & col.DataField & "] = " & col.FilterText
        Case adDBTimeStamp: tmp = tmp & "[" & col.DataField & "] = #" & col.FilterText & "#"
        Case adBoolean: tmp = tmp & "[" & col.DataField & "] = " & col.FilterText
        End Select
    End If
    X = X + 1
Next col
