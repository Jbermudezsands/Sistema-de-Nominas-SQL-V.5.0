...
Else        'CODIGO PARA GRABAR DATOS
    If Trim(Me.txtIdG.Text) = "" Then
        ...
        Me.txtIdG.SetFocus
        Exit Sub
    ElseIf Trim(UCase(Me.tdbcGrupo.Text)) = "ANTIGÜEDAD" Then
        MsgBox "Operación Cancelada, no puede agregar mas puntos a este grupo", vbInformation
        Me.txtIdG.SetFocus
        Exit Sub
    ElseIf Me.txtDescrip = "" Then
        MsgBox "La descripción del punto es requerida", vbInformation
...
sql = "SELECT [grupo], [id]  From [dbo].[PuntosGrupo] "
With rs2
    .CursorLocation = adUseClient
    .Open sql, cnx, adOpenDynamic, adLockOptimistic
End With
Me.tdbcGrupo.RowSource = rs2
Me.tdbcGrupo.BoundColumn = "Id"
...
If Not rs.EOF Then
    ...
    rs2.Find "id = " & IIf(IsNull(rs!idg), "", rs!idg), , , 1
...
If Me.CmdAgregar.Caption = "&Grabar" And Trim(Me.txtIdG.Text) <> "" Then
    rs2.MoveFirst
    rs2.Find "Id = " & val(Me.txtIdG.Text)
    Me.tdbcGrupo.Bookmark = rs2.Bookmark
End If
Exit Sub
errtxt:
