...
frmNodes.prLlave = -1
If val(rs!sufijo) = 0 Then frmNodes.prSufijo = Trim(rs!Codigo)
If val(rs!sufijo) <> 0 Then frmNodes.prSufijo = Trim(rs!sufijo) & Trim(rs!Codigo)
frmNodes.Show
End Sub
...
If Me.tdbgCuentas.Visible Then
    ...
    sql = "SELECT [CodTipoNomina], [Nomina]  From [dbo].[TipoNomina] " & _
            "Where [Activa] = 1 AND [CodTipoNomina] NOT IN ( " & _
                "SELECT [IdNomina] FROM [dbo].[_ActCuentas] " & _
                "WHERE IdActividad =" & Mid(Me.tvActividades.SelectedItem.Key, 2) & ")"
    ...
    Me.tdbcNomina.Columns(0).Width = 800
    Me.tdbcNomina.Text = ""
Else
    ...
    sql = "INSERT INTO [dbo].[_ActCuentas]([IdNomina], [Cuenta], IdActividad) " & _
            "Values('" & Trim(Me.tdbcNomina.BoundText) & "', '" & Trim(Me.txtCuenta) & "', " & Mid(Me.tvActividades.SelectedItem.Key, 2) & ")"
...
If Me.tdbgCuentas.Visible Then
    ...
    If MsgBox("¿Desea eliminar el registro?", vbYesNo) = vbYes Then
        sql = "DELETE FROM [dbo].[_ActCuentas] WHERE idactividad = " & Mid(Me.tvActividades.SelectedItem.Key, 2) & _
                " and IdNomina = '" & Trim(rs3!Id) & "'"
        cnx.Execute sql
...
sql = "DELETE FROM _Actividades WHERE idActividad = " & val(Mid(Me.tvActividades.SelectedItem.Key, 2))
...
frmNodes.prSufijo = Trim(rs!sufijo)
frmNodes.prCodigo = Trim(rs!Codigo)
...
sql = "select * from _Actividades where idactividad > 1 and idsuperior = 1"
With rs2
    If .State = adStateOpen Then .Close
    .CursorLocation = adUseClient
    .Open sql, cnx, adOpenDynamic, adLockOptimistic
...
'With tdbgActividades
'    .DataSource = rs
'    .Columns(1).DropDown = Me.tddNiveles
'End With

If rs2.RecordCount > 0 Then
    rs2.MoveFirst
    Do While Not rs2.EOF
        tvActividades.Nodes.Add , tvwLast, "A" & Trim(Str(rs2!IdActividad)), Trim(rs2!Codigo) & ".- " & Trim(rs2!actividad), 1, 2
        Call addchild(rs2!IdActividad)
        rs2.MoveNext
    Loop
End If
...
Private Sub addchild(IdActividad As Integer)
On Error GoTo erradd
Dim rs3 As New ADODB.Recordset

sql = "SELECT * From [dbo].[_Actividades] " & _
        "Where IdSuperior = " & IdActividad & " order by codigo"
...
If rs3.RecordCount > 0 Then
    ...
    Do While Not rs3.EOF
        tvActividades.Nodes.Add "A" & Trim(Str(IdActividad)), tvwChild, "A" & Trim(Str(rs3!IdActividad)), Trim(rs3!sufijo) & Trim(rs3!Codigo) & ".- " & Trim(rs3!actividad), 1, 2
        Call addchild(rs3!IdActividad)
        rs3.MoveNext
    Loop
End If
Set rs3 = Nothing
...
sql = "IdActividad = " & Mid(tvActividades.SelectedItem.Key, 2)
rs.Requery
rs.Find sql
Me.chkPago(0).Value = IIf(rs!pagacliente = 0, True, False)
Me.chkPago(1).Value = IIf(rs!pagacliente = 0, False, True)

sql = "SELECT [IdNomina] AS ID, N.[Nomina], [Cuenta] " & _
        "FROM [dbo].[_ActCuentas] C INNER JOIN [dbo].[TipoNomina] N ON C.[idnomina]= N.[CodTipoNomina] " & _
        "WHERE idactividad = " & val(Mid(Me.tvActividades.SelectedItem.Key, 2))
